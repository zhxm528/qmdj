...
## 2) 藏干表（地支拆解）（藏干表 已在step1中实现，这里可用step1获取的数据作为入参）
...
## 5) 合冲刑害破 + 干合干克：关系网表。 （合冲刑害破 已在step1中实现，这里可用step1获取的数据作为入参）
...
- 数据库：`/md/database/2_qimen_pan.sql` 、 `/md/database/11_bazi.sql`
- 配置文件： `/lib/config.ts`
