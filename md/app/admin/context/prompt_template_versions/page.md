# 核心功能
- 前端页面：`/app/admin/context/prompt_template_versions/page.tsx`
- 后台程序：`/app/api/admin/context/prompt_template_versions/route.ts`
- 数据库表结构：`/md/database/5_context.sql`
- 配置文件： `/lib/config.ts`
## 需要执行的操作
- 如果访问链接中带 “id=509e46db-5a4b-4cf6-a6ab-80c2590797a3” 类似参数，则直接查询到相应的数据，如果查不到则显示空
...
- 增加查询条件“状态”的下拉框查询
- 新增和编辑页面中，点击保存时，“模板内容”字段中的内容保存到数据库时，换行符用 `\n` 代替，编辑时，把数据回显在页面上时，`\n` 在转换为换行符
- 表格中增加“更新时间”一列
