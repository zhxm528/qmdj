# DeepSeek LLM 调用规范（八字-寒暖燥湿）

## 目标
在八字盘中生成“寒暖燥湿”板块的自然语言描述。
`app/bazi/page.tsx` 中对应 case 的 `let text` 变量应来自 LLM 返回内容。

## 寒暖燥湿
## 输入
使用“寒暖燥湿”板块的 JSON 作为 LLM 输入，该 JSON 来自 `app/api/bazi/step5.ts` 的 `Step5Result`。
...
- projectCode：bazi
- flow：flow code（如 flow.bazi.kanpan.hannuanzaoshi）
...
```json
{
  ...
  "messages": [
    ...
    { "role": "user", "content": "以下是“寒暖燥湿”板块 JSON，请生成描述：\n<JSON>" }
  ],
  "temperature": 0.4,
  "max_tokens": 200
}
...
该文本直接赋值给 `app/bazi/page.tsx` 中 `case 5` 的 `let text` 变量并显示。
