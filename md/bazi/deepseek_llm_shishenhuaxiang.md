# DeepSeek LLM 调用规范（八字-十神画像）

## 目标
在八字盘中生成“十神画像”板块的自然语言描述。
`app/bazi/page.tsx` 中对应 case 的 `let text` 变量应来自 LLM 返回内容。

## 十神画像
## 输入
使用“十神画像”板块的 JSON 作为 LLM 输入，该 JSON 来自 `app/api/bazi/step9.ts` 的 `Step9Result`。
...
```json
{
  "tenshen_profile_static": {
    ...
    "items": [
      { "tenshen_code": "正财", "count_total": 2, "score_total": 1.8, "rank_no": 1 }
    ]
  }
}
```
...
- projectCode：bazi
- flow：flow code（如 flow.bazi.kanpan.shishenhuaxiang）
...
```json
{
  ...
  "messages": [
    ...
    { "role": "user", "content": "以下是“十神画像”板块 JSON，请生成描述：\n<JSON>" }
  ],
  "temperature": 0.4,
  "max_tokens": 200
}
...
```
十神以正财与食伤为主，官杀与印次之，比劫均衡，结构偏向务实与表达。
```

该文本直接赋值给 `app/bazi/page.tsx` 中 `case 9` 的 `let text` 变量并显示。
