# DeepSeek LLM 调用规范（八字-月令与季节）

## 目标
在八字盘中生成“月令与季节”板块的自然语言描述。
`app/bazi/page.tsx` 中对应 case 的 `let text` 变量应来自 LLM 返回内容。

## 输入
使用“月令与季节”板块的 JSON 作为 LLM 输入，该 JSON 来自 `app/api/bazi/step3.ts` 的 `Step3Result`。
...
```json
{
  "month_command": {
    ...
    "season": "秋",
    "dominant_qi": "金旺",
    "supporting_elements_rank": ["金", "水", "土", "木", "火"]
  },
  "yueling_strength": {
...
- projectCode：bazi
- flow：flow code（如 flow.bazi.kanpan.yuling）
...
```json
{
  ...
  "messages": [
    ...
    { "role": "user", "content": "以下是\"月令与季节\"板块 JSON，请生成描述：\n<JSON>" }
  ],
  "temperature": 0.4,
  "max_tokens": 200
}
...
```
月令为申，季节属秋，金气当令，日主金旺，水土相助而木火偏弱。
```

该文本直接赋值给 `app/bazi/page.tsx` 中 `case 3` 的 `let text` 变量并显示。
...
## 失败处理
- LLM 请求失败时，回退为本地占位文案（保持原有逻辑）：`"月令与季节分析结果已通过下方表格和图表展示。"`。
