# DeepSeek LLM 调用规范（八字-验盘）

## 目标
在八字盘中生成“验盘”板块的自然语言描述。
`app/bazi/page.tsx` 中对应 case 的 `let text` 变量应来自 LLM 返回内容。

## 验盘
## 输入
使用“验盘”板块的 JSON 作为 LLM 输入，该 JSON 来自 `app/api/bazi/step8.ts` 的 `Step8Result`。
...
```json
{
  "consistency_check": {
    ...
    "disease_tags": ["dm_too_strong"],
    "issues": [
      { "type": "REMEDY_MISMATCH", "severity": "MEDIUM", "detail": "用神偏离主要矛盾" }
    ]
  }
}
```
...
- projectCode：bazi
- flow：flow code（如 flow.bazi.kanpan.yanpan）
...
```json
{
  ...
  "messages": [
    ...
    { "role": "user", "content": "以下是“验盘”板块 JSON，请生成描述：\n<JSON>" }
  ],
  "temperature": 0.4,
  "max_tokens": 200
}
...
```
验盘评分中等，身强偏明显，用神匹配需复核，存在个别矛盾项。
```

该文本直接赋值给 `app/bazi/page.tsx` 中 `case 8` 的 `let text` 变量并显示。
