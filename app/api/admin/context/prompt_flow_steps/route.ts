...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existingCheck = await query(
      `SELECT id, flow_id, step_order FROM prompt_flow_steps WHERE id = $1`,
      [id]
    );
    ...
    if (step_order !== undefined) {
      ...
      const checkOrder = step_order !== existing.step_order ? step_order : null;
      if (checkOrder) {
        const existingOrder = await query(
    ...
    if (version_strategy !== undefined) {
      ...
      // 如果版本策略改为latest，清除fixed_version_id
