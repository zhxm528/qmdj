...
async function getCurrentUserId(): Promise<number | null> {
  try {
    ...
    console.error("[environments] 获取用户ID失败:", error);
    return null;
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[environments] 查询列表失败:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name) {
      return NextResponse.json(
        { success: false, error: "环境名称不能为空" },
        { status: 400 }
      );
    }

    // 验证环境代号
    const validCodes = ["dev", "staging", "prod"];
    if (!validCodes.includes(code)) {
      return NextResponse.json(
        { success: false, error: "无效的环境代号，必须是 dev、staging 或 prod" },
        { status: 400 }
      );
    }
    ...
    if (name.length > 50) {
      return NextResponse.json(
        { success: false, error: "环境名称不能超过50个字符" },
        { status: 400 }
      );
    }
    ...
    console.error("[environments] 创建失败:", error);
    // 处理唯一约束错误
    if (error.code === "23505") {
      return NextResponse.json(
        { success: false, error: "环境代号已存在" },
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existingCheck = await query(
      `SELECT id, code FROM environments WHERE id = $1`,
      [id]
    );
    ...
    // 环境代号不允许修改（code字段在编辑时应该是disabled的，但这里也做保护）
    if (code !== undefined && code !== existing.code) {
      return NextResponse.json(
        { success: false, error: "环境代号不允许修改" },
        { status: 400 }
      );
    }
    ...
    console.error("[environments] 更新失败:", error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // 删除记录
    const deleteQuery = `DELETE FROM environments WHERE id = $1 RETURNING *`;
    ...
    console.error("[environments] 删除失败:", error);
    // 处理外键约束错误
    if (error.code === "23503") {
      return NextResponse.json(
        { success: false, error: "该环境被其他数据引用，无法删除" },
        { status: 400 }
      );
    }
    return NextResponse.json(
