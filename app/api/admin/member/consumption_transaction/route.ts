...
interface ConsumptionTransactionQueryParams {
  ...
  member_id?: number;
  card_no?: string;
  pay_channel?: string; // comma-separated values
...
export async function GET(request: NextRequest) {
  try {
    ...
    const params: ConsumptionTransactionQueryParams = {
      ...
      pay_channel: searchParams.get("pay_channel") || undefined,
    ...
    // 会员ID过滤
    if (params.member_id) {
      whereConditions.push(`ct.member_id = $${paramIndex}`);
    ...
    if (params.pay_channel) {
      const payChannels = params.pay_channel.split(",").filter(Boolean);
      if (payChannels.length > 0) {
        whereConditions.push(`ct.pay_channel = ANY($${paramIndex})`);
        queryValues.push(payChannels);
        paramIndex++;
      }
    }
    ...
    if (params.status) {
      ...
      if (statuses.length > 0) {
        whereConditions.push(`ct.status = ANY($${paramIndex})`);
    ...
    // 计算总数
    let countQuery = `SELECT COUNT(*) as total FROM consumption_transaction ct`;
    if (params.card_no) {
      countQuery += ` LEFT JOIN member_card mc ON ct.card_id = mc.card_id`;
    }
    ...
    let dataQuery = `
      SELECT
        ...
        ct.points_used,
        ct.points_earned,
        ct.external_order_no,
        ct.remark,
        ct.created_at AT TIME ZONE 'UTC' AS created_at
      FROM consumption_transaction ct
    `;

    if (params.card_no) {
      dataQuery += ` LEFT JOIN member_card mc ON ct.card_id = mc.card_id`;
    }

    dataQuery += ` ${whereClause} ORDER BY ct.created_at DESC`;
