...
function buildTianbashen(
  ...
  console.log(`[tianbashen] 旬首: ${xunShou}`);

  // 2. 在天盘干中找到旬首对应的宫位，这个宫位就是值符的起点
  ...
  // 如果找不到旬首对应的宫位，使用默认值
  if (zhiFuPalace === null) {
    console.warn(`[tianbashen] 未找到旬首 ${xunShou} 对应的天盘干宫位，使用默认值`);
    zhiFuPalace = path[0];
  }
  ...
  if (zhiFuPalace === 5) {
    zhiFuPalace = 2;
    console.log(`[tianbashen] 值符在中宫5，采用「出中入坤」规则，移到坤二宫2`);
  }

  console.log(`[tianbashen] 值符所在宫位: 宫${zhiFuPalace}`);

  // 3. 找到值符所在宫位在path中的索引位置
  let startIndex = path.indexOf(zhiFuPalace);
  if (startIndex === -1) {
    // 如果值符宫位不在path中（边缘情况），默认使用第一个宫位
    console.warn(`[tianbashen] 值符宫位 ${zhiFuPalace} 不在path中，使用第一个宫位`);
    startIndex = 0;
  }
  ...
  console.log(`[tianbashen] 天八神排布 (${dunType}): ${placementOrder}`);

  return placement;
}

interface TianbashenRequest {
  dunType?: string;
  hourPillar?: string;
...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as TianbashenRequest;
    const { dunType, hourPillar, tianpangan } = body || {};
    ...
    const tianbashen = buildTianbashen(dunType, hourPillar, normalizedTianpangan);
    return NextResponse.json({ tianbashen });
  } catch (error) {
    console.error("[tianbashen] 错误:", error);
