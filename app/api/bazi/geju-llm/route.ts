/**
 * 格局成局 LLM 生成 API
 * 规范见：md/bazi/deepseek_llm_geju.md
 */
...
import { Step6Result } from "../step6";

interface GejuLLMRequest {
  step6Result: Step6Result;
}

interface GejuLLMResponse {
  success: boolean;
  text?: string;
  error?: string;
}

export async function POST(request: NextRequest): Promise<NextResponse<GejuLLMResponse>> {
  try {
    const body = (await request.json()) as GejuLLMRequest;
    const { step6Result } = body;

    if (!step6Result) {
      return NextResponse.json(
        { success: false, error: "缺少 step6Result 参数" },
        { status: 400 }
      );
    }
    ...
    // 打印调用日志
    console.log("\n[bazi][geju-llm] DeepSeek API 调用入参:");
    ...
    if (!response.ok) {
      ...
      console.error(
        "[bazi][geju-llm] DeepSeek API 调用失败:",
        response.status,
        errorText
      );
      return NextResponse.json(
    ...
    console.log(
      "[bazi][geju-llm] DeepSeek API 原始返回:",
    ...
    console.log("[bazi][geju-llm] 生成的文本:", text);
    ...
    console.error("[bazi][geju-llm] 调用失败:", error);
    return NextResponse.json(
      {
        success: false,
