/**
 * 验盘 LLM 生成 API
 * 规范见：md/bazi/deepseek_llm_yanpan.md
 */
...
import { Step8Result } from "../step8";

interface YanpanLLMRequest {
  step8Result: Step8Result;
}

interface YanpanLLMResponse {
  success: boolean;
  text?: string;
  error?: string;
}

export async function POST(request: NextRequest): Promise<NextResponse<YanpanLLMResponse>> {
  try {
    const body = (await request.json()) as YanpanLLMRequest;
    const { step8Result } = body;

    if (!step8Result) {
      return NextResponse.json(
        { success: false, error: "缺少 step8Result 参数" },
        { status: 400 }
      );
    }
    ...
    // 构建 user message
    const userMessage =
      '以下是"验盘"板块 JSON，请生成描述：\n' +
      JSON.stringify(step8Result, null, 2);
    ...
    // 打印调用日志
    console.log("\n[bazi][yanpan-llm] DeepSeek API 调用入参:");
    ...
    if (!response.ok) {
      ...
      console.error(
        "[bazi][yanpan-llm] DeepSeek API 调用失败:",
        response.status,
        errorText
      );
      return NextResponse.json(
    ...
    console.log(
      "[bazi][yanpan-llm] DeepSeek API 原始返回:",
    ...
    console.log("[bazi][yanpan-llm] 生成的文本:", text);
    ...
    console.error("[bazi][yanpan-llm] 调用失败:", error);
    return NextResponse.json(
      {
        success: false,
