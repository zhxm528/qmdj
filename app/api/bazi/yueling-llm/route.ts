/**
 * 月令与季节 LLM 生成 API
 * 规范见：md/bazi/deepseek_llm_yuling.md
 */
...
import { Step3Result } from "../step3";

interface YuelingLLMRequest {
  step3Result: Step3Result;
}

interface YuelingLLMResponse {
  success: boolean;
  text?: string;
  error?: string;
}

export async function POST(request: NextRequest): Promise<NextResponse<YuelingLLMResponse>> {
  try {
    const body = (await request.json()) as YuelingLLMRequest;
    const { step3Result } = body;

    if (!step3Result) {
      return NextResponse.json(
        { success: false, error: "缺少 step3Result 参数" },
        { status: 400 }
      );
    }
    ...
    // 打印调用日志
    console.log("\n[bazi][yueling-llm] DeepSeek API 调用入参:");
    ...
    if (!response.ok) {
      ...
      console.error(
        "[bazi][yueling-llm] DeepSeek API 调用失败:",
        response.status,
        errorText
      );
      return NextResponse.json(
    ...
    console.log(
      "[bazi][yueling-llm] DeepSeek API 原始返回:",
    ...
    console.log("[bazi][yueling-llm] 生成的文本:", text);
    ...
    console.error("[bazi][yueling-llm] 调用失败:", error);
    return NextResponse.json(
      {
        success: false,
