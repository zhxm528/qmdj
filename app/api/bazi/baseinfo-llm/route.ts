/**
 * 基础盘面信息 LLM 生成 API
 * 规范见：md/bazi/deepseek_llm_baseinfo.md
 */
...
interface BaseinfoLLMRequest {
  step2Result: Step2Result;
}

interface BaseinfoLLMResponse {
  success: boolean;
  text?: string;
  error?: string;
}

export async function POST(request: NextRequest): Promise<NextResponse<BaseinfoLLMResponse>> {
  try {
    const body = (await request.json()) as BaseinfoLLMRequest;
    const { step2Result } = body;

    if (!step2Result) {
      return NextResponse.json(
        { success: false, error: "缺少 step2Result 参数" },
        { status: 400 }
      );
    }
    ...
    const flowMessages = await service.renderFlowToMessages({
      envCode,
      projectCode: "bazi",
      flow: "flow.bazi.kanpan.baseinfo",
      variables: {},
    });
    ...
    // 打印调用日志
    console.log("\n[bazi][baseinfo-llm] DeepSeek API 调用入参:");
    ...
    if (!response.ok) {
      ...
      console.error(
        "[bazi][baseinfo-llm] DeepSeek API 调用失败:",
        response.status,
        errorText
      );
      return NextResponse.json(
    ...
    console.log(
      "[bazi][baseinfo-llm] DeepSeek API 原始返回:",
    ...
    console.log("[bazi][baseinfo-llm] 生成的文本:", text);
    ...
    console.error("[bazi][baseinfo-llm] 调用失败:", error);
    return NextResponse.json(
      {
        success: false,
