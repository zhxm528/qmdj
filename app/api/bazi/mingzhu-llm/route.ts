/**
 * 定命主 LLM 生成 API
 * 规范见：md/bazi/deepseek_llm_mingzhu.md
 */
...
interface MingzhuLLMRequest {
  step1Result: Step1Result;
}

interface MingzhuLLMResponse {
  success: boolean;
  text?: string;
  error?: string;
}

export async function POST(request: NextRequest): Promise<NextResponse<MingzhuLLMResponse>> {
  try {
    const body = (await request.json()) as MingzhuLLMRequest;
    const { step1Result } = body;

    if (!step1Result) {
      return NextResponse.json(
        { success: false, error: "缺少 step1Result 参数" },
        { status: 400 }
      );
    }
    ...
    // 打印调用日志
    console.log("\n[bazi][mingzhu-llm] DeepSeek API 调用入参:");
    ...
    if (!response.ok) {
      ...
      console.error(
        "[bazi][mingzhu-llm] DeepSeek API 调用失败:",
        response.status,
        errorText
      );
      return NextResponse.json(
    ...
    console.log(
      "[bazi][mingzhu-llm] DeepSeek API 原始返回:",
    ...
    console.log("[bazi][mingzhu-llm] 生成的文本:", text);
    ...
    console.error("[bazi][mingzhu-llm] 调用失败:", error);
    return NextResponse.json(
      {
        success: false,
